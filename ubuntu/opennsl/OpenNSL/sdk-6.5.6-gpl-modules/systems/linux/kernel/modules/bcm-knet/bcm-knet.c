...
bkn_dnx_packet_parse_ftmh(uint8_t hdr_buff[], bkn_dnx_packet_info *packet_info)
{
    ...
    DBG_DUNE(("FTMH(%d) Packet-size %d Source-system-port 0x%x Traffic-class %d\n",
              packet_info->ntwrk_header_ptr, packet_info->ftmh.packet_size, packet_info->ftmh.src_sys_port, packet_info->ftmh.prio));
...
static uint32_t
bkn_dnx_packet_header_parse(uint8 *buff, bkn_dnx_packet_info *packet_info)
{
    uint32_t rc = 0;
    uint32_t header_ptr;
    ...
    memcpy(hdr_buff, buff, BKN_DNX_HDR_MAX_SIZE);
    header_ptr = 0;
...
bkn_do_api_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcb_chain->dcb_cur < dcb_chain->dcb_cnt) {
        ...
        if (sinfo->dcb_type == 28) {
            memset(&packet_info, 0, sizeof(bkn_dnx_packet_info));
            bkn_dnx_packet_header_parse(pkt,&packet_info);
            ...
            bkn_dnx_bitstream_set_field((uint32_t *)dcb, 258, 10, packet_info.internal.trap_id);
...
bkn_do_skb_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcbs_done < budget) {
        ...
        if (sinfo->dcb_type == 28) {
            memset(&packet_info, 0, sizeof(bkn_dnx_packet_info));
            bkn_dnx_packet_header_parse(skb->data,&packet_info);
            bkn_dnx_bitstream_set_field((uint32_t *)dcb, 281, 5, packet_info.ftmh.src_sys_port);
            bkn_dnx_bitstream_set_field((uint32_t *)dcb, 296, 12, packet_info.internal.vsi);
            bkn_dnx_bitstream_set_field((uint32_t *)dcb, 258, 10, packet_info.internal.trap_id);
...
/* OPENNSL_FIXUP <FP-247203 SDK-112907> - Start */
...
#endif
/* OPENNSL_FIXUP <FP-247203 SDK-112907> - End */
...
static const struct net_device_ops bkn_netdev_ops = {
    ...
    .ndo_set_mac_address = bkn_set_mac_address, /* OPENNSL_FIXUP <FP-247203 SDK-112907> */
    .ndo_do_ioctl        = NULL,
    .ndo_tx_timeout      = NULL,
...
bkn_knet_netif_create(kcom_msg_netif_create_t *kmsg, int len)
{
    ...
    if (priv->type == KCOM_NETIF_T_PORT) {
       ...
       if ((sinfo->dcb_type == 28) && (priv->type == KCOM_NETIF_T_VLAN)) {
